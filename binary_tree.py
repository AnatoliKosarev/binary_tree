...
class BinaryTree:
    ...
    def add(self, new_node: Node):
        ...
        while current_node:
            if new_node.value == current_node.value:
                raise ValueError('Node with the same value already exists!')
    ...
    def delete(self, value: int):
        ...
        if to_delete.right and to_delete.left:
            ...
            if to_delete == to_delete_parent.left:
                to_delete_parent.left = to_delete.right or to_delete.left  # because we don't know from which side to_delete node has child
            elif to_delete == to_delete_parent.right:
                to_delete_parent.right = to_delete.right or to_delete.left
            else:
                # if to_delete == self.head - change head to to_delete's child
